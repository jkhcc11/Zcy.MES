...
namespace Zcy.MongoDB.PurchaseSale
{
    ...
    public class PurchaseOrderRepository : BaseMongodbRepository<PurchaseOrder, long>, IPurchaseOrderRepository
    {
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<List<PurchaseOrderTotalsTemp>> PurchaseOrderTotalsAsync(IQueryable<PurchaseOrder> queryable)
        {
            var tempList = await ToMongoQueryable(queryable)
                .GroupBy(a => a.OrderDate)
                .Select(a => new PurchaseOrderTotalsTemp()
                {
                    TotalsDate = a.Key,
                    OrderPrice = a.Sum(b => b.OrderPrice),
                    SumProductPrice = a.Sum(b => b.SumProductPrice),
                })
                .ToDynamicListAsync<PurchaseOrderTotalsTemp>();
