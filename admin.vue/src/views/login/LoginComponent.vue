  ...
  <n-el>
    ...
    <div v-else class="m-login-container">
      <div class="header">
        ...
        <div class="mt-4 text-lg font-bold text-white"> ZCYMES Admin </div>
      </div>
      <div class="content">
    ...
    setup() {
      ...
      const onLogin = () => {
        ...
        post({
          ...
          .then((res: Response) => {
            userStore.saveToken(res.data as UserState).then(() => {
              //保存用户信息
              userStore.saveUser()

              let redirect = route.query.redirect as string
              if (redirect.indexOf('login-with-token?token=') != -1) {
                //token过来的 直接主页
                redirect = '/'
              }
