<template>
  <div class="main-container">
    <n-card :title="editData.title" style="margin-bottom: 16px">
      ...
      <DataForm
        ref="submitForm"
        :options="CreatePurchaseOrderFormOptions"
        :form-config="{
          labelWidth: 100,
          size: 'medium',
          labelAlign: 'right',
      ...
      <template #action>
        ...
        <n-gradient-text type="error">
          商品数量：{{ orderDetailData.length }} 订单金额：{{ editData.orderPrice ?? '-' }}
        </n-gradient-text>
      </template>
    </n-card>
    ...
    >
      <template #content>
        <CommonTableSelector
          ...
          :searchFormOptions="PurchaseProductFilterForm"
...
<script lang="ts">
  import { PurchaseOrderApi, productApi } from '@/api/url'
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const orderDetailData = reactive<CreateSaleOrderDetailInput[]>([])
          ...
          },
          {
            ...
            render(row: any, index: number) {
              return h(NInputNumber, {
                ...
                placeholder: '产品单价',
                onUpdateValue(newVal: any) {
                  orderDetailData[index].unitPrice = newVal
                },
              })
            },
          },
      ...
      //创建采购订单
      const CreatePurchaseOrderFormOptions = [
        {
          key: 'id',
          value: ref(null),
          hidden: true,
      ...
      const calculateSumPrice = () => {
        const tempPrice =
          (editData.freightPrice ?? 0) +
          orderDetailData.reduce((acc, item) => acc + parseFloat(item.sumPrice + ''), 0)
      ...
      return {
        ...
        productApi,
        PurchaseOrderApi,
        CreatePurchaseOrderFormOptions,
        editLoadingRef,
        editData,
        vodTableSelectorRef,
        modalDialogRef,
