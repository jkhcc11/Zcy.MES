<template>
  <CommonQueryList
    ...
    :queryApi="ShipmentOrderApi.query"
...
<script lang="ts">
  ...
  import { ShipmentOrderApi } from '@/api/url'
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const onBtnClick = {
        //创建
        create: function () {
          router.push('/order/shipment-order-edit')
        },
        //详情
        detail: function (rowData: any) {
          ...
          get({
            url: ShipmentOrderApi.detail + '/' + rowData.id,
          })
            .then((res) => {
        ...
        delete: function (rowData: any) {
          naiveDialog.warning({
            ...
            onPositiveClick: () => {
              sendDelete({
                url: ShipmentOrderApi.delete + '/' + rowData.id,
                data: {},
              })
                .then((res) => {
                  doRefresh()
