...
import { queryPageProductApi, banOrEnableProductApi } from '@/services/product'
...
const queryData = async () => {
  ...
  // 发送请求
  const res = await queryPageProductApi(queryParams)
  // 发送请求后，重置标记
...
//枚举显示
const getProductTypeItem = (enumItem: ProductTypeEnum) => {
  return ProductTypeEnumList.filter((item) => item.value == enumItem)[0]
}
...
function onBan(item: any) {
  uni.showModal({
    content: `是否禁用【${item.productName}】?`,
    confirmColor: '#ff9240',
    success: async (res) => {
      if (res.confirm) {
        const res = await banOrEnableProductApi(item.id)
        if (res.isSuccess) {
...
//创建
function onCreate() {
  uni.navigateTo({ url: `product-create` })
}

//筛选
const tempStatusArray = PublicStatusEnumList.filter((it) =>
  [PublicStatusEnum.正常, PublicStatusEnum.禁用].includes(it.value),
)
  ...
  <view class="viewport">
    ...
    >
      <view class="card" v-for="dataItem in dataList" :key="dataItem.id">
        <!-- 订单信息 -->
        <view class="status">
          <text>分类名: {{ dataItem.productTypeName }}</text>
          <uni-tag
            :text="getStatusItem(dataItem.productStatus).text"
            :type="getStatusItem(dataItem.productStatus).tagType"
            :inverted="true"
            size="small"
          />
        </view>
        ...
        <view class="payment">
          <view class="payment_left">
            <text class="quantity" v-if="!dataItem.isLoose && dataItem.unit"
              >规格：{{ dataItem.specCount }}{{ dataItem.unit }}/{{ dataItem.spec }}</text
            >
            <text class="quantity" v-if="dataItem.isLoose && dataItem.unit"
              >规格：{{ dataItem.unit }}</text
            >

            <text class="quantity" v-if="dataItem.productRemark"
              >备注：{{ dataItem.productRemark }}</text
            >
          </view>
          <view class="payment_right">
            <uni-tag
              :text="getProductTypeItem(dataItem.productType).text"
              :type="getProductTypeItem(dataItem.productType).tagType"
              :inverted="true"
              size="small"
            />
          </view>
        ...
        <view class="action">
          <view class="action-left">
            <!-- <text>分类：{{ dataItem.productTypeName }}</text> -->
          </view>
          <view class="action-right">
            <view
              class="button warning"
              @tap="onBan(dataItem)"
              v-if="dataItem.productStatus === PublicStatusEnum.正常"
            >
              禁用
            </view>
            <view
              ...
              @tap="onOpen(dataItem)"
              v-if="dataItem.productStatus === PublicStatusEnum.禁用"
            >
              启用
            </view>
          </view>
