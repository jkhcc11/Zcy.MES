...
import { PublicStatusEnumList, CommonPageSize, PublicStatusEnum } from '@/services/constants'
import { queryPageProductTypeApi, banOrEnableApi } from '@/services/product'
...
const queryData = async () => {
  ...
  // 发送请求
  const res = await queryPageProductTypeApi(queryParams)
  // 发送请求后，重置标记
...
function onBan(item: any) {
  uni.showModal({
    content: `是否禁用【${item.typeName}】?`,
    confirmColor: '#ff9240',
    success: async (res) => {
      if (res.confirm) {
        const res = await banOrEnableApi(item.id)
        if (res.isSuccess) {
...
//创建
function onCreate() {
  uni.navigateTo({ url: `product-type-create` })
}
  ...
  <view class="viewport">
    ...
    >
      <view class="card" v-for="dataItem in dataList" :key="dataItem.id">
        <!-- 订单信息 -->
        <view class="status">
          <!-- <text>{{ dataItem.typeName }}</text> -->
          <uni-tag
            :text="getStatusItem(dataItem.typeStatus).text"
            :type="getStatusItem(dataItem.typeStatus).tagType"
            :inverted="true"
            size="small"
          />
        </view>
        ...
        <view class="action">
          ...
          <view class="action-right">
            <view
              ...
              @tap="onBan(dataItem)"
              v-if="dataItem.typeStatus === PublicStatusEnum.正常"
            >
              禁用
            </view>
            <view
              ...
              @tap="onOpen(dataItem)"
              v-if="dataItem.typeStatus === PublicStatusEnum.禁用"
            >
              启用
            </view>
          </view>
