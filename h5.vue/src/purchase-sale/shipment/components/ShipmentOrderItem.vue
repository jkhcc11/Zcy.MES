...
import { CommonPageSize } from '@/services/constants'
import type { QueryPageShipmentOrderInput } from '@/types/new-order'
import { getShipmentOrderTotalsApi, queryPageShipmentOrderApi } from '@/services/new-order'
...
const totalRef = reactive({
  //总出货数量
  sumProductCount: null,
})

// 请求参数
const queryParams: Required<QueryPageShipmentOrderInput> = {
  page: 1,
...
const querOrderData = async () => {
  ...
  // 发送请求
  const res = await queryPageShipmentOrderApi(queryParams)
  const resTotal = await getShipmentOrderTotalsApi(queryParams)
  ...
  if (res.isSuccess) {
    //汇总赋值
    totalRef.sumProductCount = resTotal.data.sumProductCount
...
const onDetail = (id: string) => {
  uni.navigateTo({ url: `/purchase-sale/shipment/detail?id=${id}` })
}
        ...
        <uni-row class="demo-uni-row">
          <uni-col :span="24">
            <uni-section
              class="mb-10"
              title="总出货量"
              :sub-title="`${totalRef.sumProductCount ?? '-'} `"
              type="line"
            ></uni-section>
          </uni-col>
        </uni-row>
    ...
    >
      <view class="card" v-for="order in orderList" :key="order.id">
        ...
        <view class="payment">
          ...
          <view class="payment_right" v-if="order.pickUpUser">
            <text>提货人</text>
            <text class="amount">{{ order.pickUpUser }}</text>
          </view>
        </view>

        <!-- 订单操作按钮 -->
        <view class="action">
          <view class="action-left"> 客户：{{ order.supplierClientName }} </view>
