...
import { ClientTypeEnum, ProductTypeEnum } from '@/services/constants'
import { createShipmentOrderApi } from '@/services/new-order'
...
onShow(() => {
  if (useProductSelected.selectedProducts && useProductSelected.selectedProducts.length > 0) {
    // 获取去重后的产品列表
    const uniqueProducts = useProductSelected.selectedProducts.filter((item) => {
...
const onCreateOrder = async () => {
  ...
  try {
    ...
    const res = await createShipmentOrderApi(orderForm)
    uni.hideLoading()
...
function openSelectedProduct() {
  // 跳转到结算页
  uni.navigateTo({ url: `../product?productType=${ProductTypeEnum.加工产品}` })
  ...
  <scroll-view enable-back-to-top scroll-y class="scroll-view">
    ...
    <uni-forms :model="orderForm" calss="form">
      <view class="form-content">
        ...
        <view name="pickUpUser" class="form-item">
          <text class="label">提货人</text>
          <input class="input" placeholder="提货人" :maxlength="8" v-model="orderForm.pickUpUser" />
        </view>
            ...
            <view class="top_row">
              <view class="meta">
                ...
                <text class="attrsText ellipsis" v-if="item.unit && item.isLoose">
                  数量：{{ item.count }}{{ item.unit }}</text
                >
                <text class="attrsText ellipsis" v-if="item.spec && !item.isLoose">
                  数量：{{ item.count }}{{ item.spec }}({{ item.specCount }}{{ item.unit }})</text
                >
    ...
    >
      <text class="text">出货量:</text>
      <text class="amount">{{ totalRef.totalCount }}</text>
