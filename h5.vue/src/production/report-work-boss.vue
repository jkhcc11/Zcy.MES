...
import { CommonPageSize, PublicStatusEnumList, PublicStatusEnum } from '@/services/constants'
import { useEmployeeStore } from '@/stores/modules/employeeStore'
import { queryPageReportWorkForAdminApi, getReportWorkTotalsApi } from '@/services/production'
...
const queryParams: Required<any> = {
  ...
  employeeId: '',
  productProcessId: '',
  reportWorkStatus: PublicStatusEnum.正常,
...
const queryData = async () => {
  ...
  if (res.isSuccess) {
    ...
    totalRef.totalActualSettlementPrice = resTotal.data.totalActualSettlementPrice
    totalRef.totalReceivableSettlementPrice = resTotal.data.totalReceivableSettlementPrice
        ...
        <uni-row class="demo-uni-row">
          ...
          <uni-col :span="6">
            <uni-section
              class="mb-10"
              title="应结"
              :sub-title="`${totalRef.totalReceivableSettlementPrice ?? '-'} 元`"
              type="line"
            ></uni-section>
          </uni-col>
          <uni-col :span="6">
            <uni-section
              class="mb-10"
              title="实际"
              :sub-title="`${totalRef.totalActualSettlementPrice ?? '-'} 元`"
              type="line"
            ></uni-section>
          </uni-col>
        </uni-row>
